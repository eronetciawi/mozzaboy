...
export const Maintenance: React.FC = () => {
  ...
  // CSV Import State
  const [activeImportTable, setActiveImportTable] = useState<'products' | 'inventory' | 'categories' | 'outlets' | 'staff' | 'wip_recipes' | 'expenses' | 'purchases' | null>(null);
  ...
  return (
    <div className="p-4 md:p-8 h-full overflow-y-auto custom-scrollbar bg-slate-50/50 relative pb-32">
      
      {/* GLOBAL MAINTENANCE TOAST */}
      {toast.type && (
      ...
      <div className="max-w-6xl mx-auto pb-24">
        ...
        <div className="mb-12">
           <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
              <div>
                <h2 className="text-2xl md:text-3xl font-black text-slate-800 uppercase tracking-tighter">Master CSV Control</h2>
                <p className="text-slate-500 font-medium text-[10px] md:text-xs italic uppercase tracking-widest">Maintenance Data Massal via Spreadsheet</p>
              </div>
              <div className="bg-indigo-600 text-white px-5 py-2 rounded-2xl text-[9px] font-black uppercase tracking-widest shadow-lg shadow-indigo-500/20">ENGINE V2.5 READY</div>
           </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                { id: 'purchases', label: 'Riwayat Belanja', icon: '🚛' },
              ].map(table => (
                <div key={table.id} className="bg-white p-6 rounded-[40px] border-2 border-slate-100 shadow-sm flex flex-col gap-5 hover:border-indigo-400 transition-all group">
                   <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center text-2xl shadow-inner group-hover:bg-indigo-50 transition-colors">{table.icon}</div>
                      <div>
                        <h4 className="text-[11px] font-black text-slate-800 uppercase tracking-widest">{table.label}</h4>
                        <p className="text-[7px] font-bold text-slate-400 uppercase">Tabel: {table.id}</p>
                      </div>
                   </div>
                   <div className="grid grid-cols-2 gap-2">
                      <button 
                        onClick={() => exportTableToCSV(table.id as any)}
                        className="py-3 bg-slate-50 text-slate-600 rounded-2xl text-[8px] font-black uppercase hover:bg-indigo-50 hover:text-indigo-600 transition-all border border-slate-100"
                      >
                        Backup
                      </button>
                      <button 
                        onClick={() => handleCSVImportClick(table.id as any)}
                        className="py-3 bg-slate-900 text-white rounded-2xl text-[8px] font-black uppercase hover:bg-orange-600 transition-all shadow-xl shadow-slate-900/10 active:scale-95"
                      >
                        Restore
                      </button>
                   </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white p-10 rounded-[56px] border-2 border-orange-100 shadow-xl shadow-orange-500/5 relative overflow-hidden flex flex-col">
             ...
             <div className="relative z-10 mb-8">
                <h3 className="text-2xl md:text-3xl font-black uppercase tracking-tighter text-slate-900">Branch Replicator</h3>
                <p className="text-[10px] font-black text-orange-600 uppercase tracking-[0.3em] mt-2">Duplikasi Struktur Menu & Inventory ke Cabang Baru</p>
             </div>
             
             <p className="text-[10px] md:text-xs text-slate-500 font-medium leading-relaxed mb-10 relative z-10 max-w-lg uppercase">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12 relative z-10">
                <div>
                   <label className="block text-[9px] font-black text-slate-400 uppercase mb-4 ml-1 tracking-widest">Dari Cabang (Sumber)</label>
                   <select 
                ...
                <div>
                   <label className="block text-[9px] font-black text-slate-400 uppercase mb-4 ml-1 tracking-widest">Ke Cabang (Target)</label>
                   <select 
                      className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[28px] font-black text-sm text-slate-800 outline-none focus:border-orange-500 transition-all cursor-pointer shadow-inner" 
                      value={cloneToId} 
             ...
             <button 
                ...
                className={`w-full py-8 rounded-[32px] font-black text-[10px] md:text-xs uppercase tracking-[0.4em] transition-all relative z-10 shadow-2xl ${(!cloneFromId || !cloneToId || cloneFromId === cloneToId) ? 'bg-slate-100 text-slate-300' : 'bg-slate-900 text-white hover:bg-orange-600 shadow-orange-900/20 active:scale-[0.98]'}`}
             >
          ...
          <div className="bg-white p-10 rounded-[56px] border-2 border-slate-100 shadow-sm flex flex-col">
            ...
            <h3 className="text-xl md:text-2xl font-black text-slate-800 uppercase tracking-tighter mb-3 leading-none">Full Master <br/>(Mode JSON)</h3>
            <p className="text-[9px] text-slate-400 font-bold leading-relaxed mb-12 uppercase tracking-widest">
              Backup seluruh ekosistem (termasuk relasi kompleks & pengaturan global) dalam satu paket file terenskripsi JSON.
            </p>
            <div className="space-y-4 mt-auto">
              <button onClick={() => fileInputRef.current?.click()} className="w-full py-5 bg-slate-900 text-white rounded-3xl font-black text-[10px] uppercase shadow-xl shadow-slate-900/20 active:scale-95 transition-all">Impor Master JSON</button>
              <button onClick={exportData} className="w-full py-5 text-slate-400 font-black text-[8px] uppercase tracking-[0.2em] hover:text-slate-800 transition-all border-2 border-dashed border-slate-200 rounded-3xl">Unduh Backup JSON 💾</button>
        ...
        <div className="mt-8 bg-white p-10 rounded-[56px] border-2 border-slate-100 shadow-sm flex flex-col">
            <div className="flex justify-between items-start mb-10">
               <div className="flex items-center gap-5">
                  <div className={`w-14 h-14 md:w-16 md:h-16 rounded-[28px] flex items-center justify-center text-3xl shadow-inner ${isCloudConnected ? 'bg-indigo-50 text-indigo-600' : 'bg-slate-50 text-slate-300'}`}>
                     {isCloudConnected ? '☁️' : '🔌'}
                  </div>
                  <div>
                    <h3 className="text-xl md:text-2xl font-black text-slate-800 uppercase tracking-tighter leading-none">Cloud Connector</h3>
                    <div className="flex items-center gap-2 mt-2">
                       <div className={`w-2.5 h-2.5 rounded-full ${isCloudConnected ? 'bg-green-500 animate-pulse' : 'bg-slate-300'}`}></div>
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{isCloudConnected ? 'Connected & Live Sync' : 'Standalone / Offline Mode'}</p>
                    </div>
                  </div>
               </div>
               <button onClick={syncToCloud} className="bg-slate-50 border-2 px-6 py-3 rounded-2xl text-[8px] font-black uppercase hover:border-indigo-500 transition-all shadow-sm">Sync Now 🔄</button>
            </div>

            <div className="space-y-8">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                     <label className="block text-[9px] font-black text-slate-400 uppercase mb-3 ml-2">Supabase Project URL</label>
                     <input 
                        type="text" 
                  ...
                  <div>
                     <label className="block text-[9px] font-black text-slate-400 uppercase mb-3 ml-2">Supabase Anon Key</label>
                     <input 
                        type="password" 
                        className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[28px] font-bold text-xs focus:border-indigo-500 outline-none transition-all shadow-inner"
               ...
               <div className="flex gap-4">
                  <button 
                    ...
                    className="flex-1 py-5 bg-slate-900 text-white rounded-[28px] font-black text-[10px] uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-2xl"
                  >
                  ...
                  {supabaseConfig.isEnabled && (
                    <button 
                      ...
                      className="px-8 py-5 bg-red-50 text-red-500 rounded-[28px] font-black text-[10px] uppercase border-2 border-red-100 hover:bg-red-600 hover:text-white transition-all"
                    >Offline</button>
                  )}
               </div>
            </div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8">
           ...
           <div className="bg-red-50 p-10 rounded-[56px] border-4 border-red-100 relative overflow-hidden flex flex-col justify-center border-dashed">
              ...
              <h3 className="text-xl md:text-2xl font-black text-red-600 uppercase tracking-tighter mb-2 leading-none">Factory Global <br/>Wipe Data</h3>
              <p className="text-[9px] text-red-800/60 font-black uppercase tracking-widest mb-12">Tindakan fatal: Mengosongkan seluruh isi database Cloud untuk semua cabang (Termasuk Master Data).</p>
              <button onClick={() => setShowGlobalResetConfirm(true)} className="w-full py-6 bg-red-600 text-white rounded-[28px] font-black text-[10px] md:text-[12px] uppercase tracking-[0.3em] shadow-2xl shadow-red-600/30 active:scale-95 transition-all">FACTORY RESET 🧨</button>
           </div>
        </div>
      </div>
      ...
      {showCloneConfirm && (
        <div className="fixed inset-0 z-[1000] bg-slate-900/95 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="bg-white rounded-[56px] w-full max-w-lg p-12 shadow-2xl text-center animate-in zoom-in-95 duration-300">
             ...
             <h3 className="text-2xl md:text-3xl font-black text-slate-800 uppercase tracking-tighter mb-4">Confirm Replicate?</h3>
             <p className="text-slate-500 text-[9px] md:text-[10px] font-black leading-relaxed uppercase tracking-widest">
      ...
      {showGlobalResetConfirm && (
        <div className="fixed inset-0 z-[1000] bg-red-950/95 backdrop-blur-2xl flex items-center justify-center p-4">
          <div className="bg-white rounded-[56px] w-full max-w-lg p-12 shadow-2xl text-center animate-in zoom-in-95">
            ...
            <h3 className="text-2xl md:text-3xl font-black text-red-600 uppercase tracking-tighter">WIPE TOTAL CLOUD?</h3>
            <p className="text-slate-500 text-[9px] md:text-[10px] font-black uppercase mt-8 mb-12 leading-relaxed tracking-widest">
      ...
      {showOutletResetConfirm && (
        <div className="fixed inset-0 z-[1000] bg-slate-900/95 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="bg-white rounded-[56px] w-full max-w-lg p-12 shadow-2xl text-center animate-in zoom-in-95">
            <div className="text-6xl mb-8">🧹</div>
            <h3 className="text-2xl md:text-3xl font-black text-slate-800 uppercase tracking-tighter">Konfirmasi Safe-Wipe</h3>
            ...
            <div className="flex flex-col gap-4">
              <button onClick={() => { 
                setToast({ message: `Membersihkan log data ${selectedOutlet?.name}...`, type: 'info' });
                resetOutletData(targetOutletId); 
                setShowOutletResetConfirm(false); 
                setTargetOutletId(''); 
                setToast({ message: "Log operasional berhasil dibersihkan.", type: 'success' });
              }} className="w-full py-6 bg-orange-600 text-white rounded-[28px] font-black text-xs uppercase tracking-[0.3em] shadow-2xl">YA, BERSIHKAN LOG CABANG</button>
