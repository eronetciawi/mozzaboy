    ...
    <div className="p-4 md:p-8 h-full overflow-y-auto custom-scrollbar bg-[#fcfdfe] pb-40">
      ...
      {(isInitialLoading || (products.length === 0 && isFetching)) && (
        <div className="mb-6 p-4 bg-slate-900 rounded-[32px] text-white flex items-center justify-between shadow-2xl border-b-4 border-orange-500 animate-in slide-in-from-top-4 duration-500">
           <div className="flex items-center gap-4">
              <div className="w-10 h-10 bg-white/10 rounded-2xl flex items-center justify-center text-xl animate-spin">🔄</div>
              <div>
                 <h4 className="text-[10px] font-black uppercase tracking-widest leading-none mb-1 text-orange-400">Menghubungkan ke Cloud...</h4>
                 <p className="text-[11px] font-bold opacity-80 uppercase leading-tight">Mohon tunggu, memuat aset bisnis pertama kali.</p>
              </div>
           </div>
        </div>
      )}
                   ...
                   {status.isOpen ? (
                     <div className="pt-4 border-t border-slate-50 space-y-3">
                        <div className="flex items-center justify-between">
                           <div className="flex items-center gap-2"><span className="text-xs">⏰</span><span className="text-[8px] font-black text-slate-400 uppercase">Jam Buka</span></div>
                           <p className="text-[10px] font-black text-slate-800">{status.openTime} WIB</p>
                        </div>
                        <div className="flex items-center justify-between">
                           <div className="flex items-center gap-2"><span className="text-xs">👥</span><span className="text-[8px] font-black text-slate-400 uppercase">Kru Aktif</span></div>
         ...
         <div className="bg-slate-900 p-8 rounded-[48px] text-white shadow-2xl flex flex-col h-full overflow-hidden">
            ...
            <div className="space-y-3 flex-1 overflow-y-auto custom-scrollbar pr-1">
               {latestTransactions.map(tx => (
                  <button key={tx.id} onClick={() => setViewingTransaction(tx)} className="w-full flex justify-between items-center p-5 bg-white/5 hover:bg-white/10 rounded-[28px] border border-white/5 transition-all text-left group active:scale-[0.98]">
                     <div className="min-w-0 flex-1">
                        <p className="text-[11px] font-black uppercase truncate group-hover:text-orange-400 transition-colors">{tx.items.map(i => i.product.name).join(', ')}</p>
                        <div className="flex items-center gap-2 mt-1">
                           ...
                           <p className="text-[8px] text-slate-500 font-bold uppercase tracking-tighter">{new Date(tx.timestamp).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})} • #{tx.id.slice(-6).toUpperCase()}</p>
                        </div>
                     </div>
      ...
      {viewingTransaction && (
        <div className="fixed inset-0 z-[500] bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4" onClick={() => setViewingTransaction(null)}>
           <div className="w-full max-w-xs md:max-w-sm flex flex-col gap-4 animate-in zoom-in-95 relative" onClick={e => e.stopPropagation()}>
              <div ref={receiptRef} className="bg-white p-6 md:p-8 rounded-[28px] shadow-2xl text-slate-900 border border-slate-100 overflow-hidden relative">
                 <div className="text-center border-b-2 border-dashed border-slate-200 pb-5 mb-5">
                    ...
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">{outlets.find(o => o.id === viewingTransaction.outletId)?.name || 'Store Hub'}</p>
                 </div>
                 <div className="space-y-4 mb-5">
                    <div className="flex justify-between text-[8px] font-black uppercase text-slate-400 tracking-widest">
                       ...
                       <span>{new Date(viewingTransaction.timestamp).toLocaleString('id-ID')}</span>
                    </div>
                       ...
                       {viewingTransaction.items.map((item, idx) => (
                          <div key={idx} className="flex justify-between items-start">
                             <div className="flex-1 pr-4">
                                ...
                                <p className="text-[8px] font-bold text-slate-400 mt-0.5">{item.quantity} x Rp {(item.product.outletSettings?.[viewingTransaction.outletId]?.price || item.product.price || 0).toLocaleString()}</p>
                             </div>
              ...
              <div className="flex gap-2">
                 <button onClick={downloadReceipt} className="flex-1 py-4 text-white rounded-[20px] font-black text-[10px] uppercase tracking-widest shadow-2xl active:scale-95 transition-all" style={{ backgroundColor: brandConfig.primaryColor || '#f97316' }}>SAVE IMAGE</button>
                 <button onClick={() => setViewingTransaction(null)} className="flex-1 py-4 bg-white text-slate-900 rounded-[20px] font-black text-[10px] uppercase tracking-widest shadow-xl active:scale-95 transition-all">CLOSE</button>
              </div>
           </div>
        </div>
      )}
